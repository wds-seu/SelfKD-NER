...
class NERDataset(Dataset):
    def __init__(self, task_type, features, mode, **kwargs):
        ...
        self.mix_token_ids = [torch.tensor(example.token_ids).long() for example in mix_features]
        self.mix_attention_masks = [torch.tensor(example.attention_masks).float() for example in mix_features]
        self.mix_token_type_ids = [torch.tensor(example.token_type_ids).long() for example in mix_features]



        self.labels = None
        self.mix_labels = None
        self.start_ids, self.end_ids = None, None
        self.ent_type = None
        self.pseudo = None
        if mode == 'train':
            self.pseudo = [torch.tensor(example.pseudo).long() for example in features]
            if task_type == 'crf' or task_type == 'lstm_crf':
    ...
    def __getitem__(self, index):
        data = {'token_ids': self.token_ids[index],
                ...
                'mix_token_ids': self.mix_token_ids[index],
                'mix_attention_masks': self.mix_attention_masks[index],
                'mix_token_type_ids':self.mix_token_type_ids[index]}
        ...
        if self.labels is not None:
            ...
            data['mix_labels'] = self.mix_labels[index]
