...
class SpanModel(BaseModel):
    def __init__(self,
        ...
        # 语义判别网络任务层 
        self.mix_fc = nn.Linear(out_dims, 2)
    ...
    def forward(self,
        ...
        mix_bert_outputs = self.bert_module(
            input_ids = mix_token_ids,
            attention_masks = mix_attention_masks,
            token_type_ids = mix_token_type_ids
        )
        ...
        mix_seq_out = mix_bert_outputs[0]
        mix_seq_out = self.mid_linear(mix_seq_out)

        start_logits = self.start_fc(seq_out)
        end_logits = self.end_fc(seq_out)

        mix_logits = self.mix_fc(mix_seq_out)
        ...
        if start_ids is not None and end_ids is not None and self.training:
            ...
            if pseudo is not None:
                # (batch,)
                start_loss = self.criterion(start_logits, start_ids.view(-1)).view(-1, 120).mean(dim=-1)
                end_loss = self.criterion(end_logits, end_ids.view(-1)).view(-1, 120).mean(dim=-1)
                ...
                mix_loss = self.criterion(mix_logits, mix_labels)

            loss = start_loss + end_loss + mix_loss
